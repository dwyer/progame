...
TMP_Tilemap *TMP_LoadTilemap(const char *filename) {
    ...
    for (i = 0; i < result->depth; i++) {
        result->layers[i] = SDL_CreateRGBSurface(SDL_HWSURFACE, result->width *
                TILE_SZ, result->height * TILE_SZ, 32, 0, 0, 0, 0);
        SDL_SetColorKey(result->layers[i], SDL_SRCCOLORKEY,
                        SDL_MapRGB(result->layers[i]->format, 255, 0, 255));
        ...
        for (j = 0; j < result->height; j++) {
            for (k = 0; k < result->width; k++) {
                ...
                dst.x = k * TILE_SZ;
                dst.y = j * TILE_SZ;
                SDL_BlitSurface(result->tileset, &src, result->layers[i], &dst);
            }
        }
    }
    return result;
